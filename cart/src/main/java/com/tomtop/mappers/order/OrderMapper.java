...
import com.tomtop.dto.order.DropShipOrderMessage;
import com.tomtop.dto.order.Order;
import com.tomtop.dto.order.OrderApiVo;
import com.tomtop.dto.order.OrderCommission;
import com.tomtop.dto.order.OrderDetail;
import com.tomtop.dto.order.OrderIdStatusId;
import com.tomtop.dto.order.OrderReportForm;
import com.tomtop.dto.order.OrderWithDtail;
...
//			+ "and cmemberemail = #{email} </if><if test=\"srcStatus != null\">and istatus = #{srcStatus} </if></script>")
	...
	@Update({
		"<script>",
		"update t_order set ishow = #{type} where cordernumber in ",
	...
	int updateOrderForDel(@Param("email")String email, 
			@Param("type")Integer showType, 
			@Param("ids")List<String> orderNumbers,
			@Param("siteid")Integer siteid);
	
	@Update({
		"<script>",
		"update t_order set cpaymentid=#{1},cpaymenttype=#{1} where iid=#{0} ",
		"</script>"
	})
	int updateOrderPaymentId(Integer iid, String paymentId);

	String getorderIdWithoutComment(@Param("email")String email,@Param("listingid")String listingid);
