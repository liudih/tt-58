...
import com.tomtop.dto.product.ProductBundleSale;
import com.tomtop.dto.product.ProductMultiattributeItem;
import com.tomtop.mappers.product.ProductBundleSaleMapper;
import com.tomtop.mappers.product.ProductMultiattributeSkuMapper;
...
import com.tomtop.valueobjects.product.spec.BundleProductSpec;
		...
		for (String key : mutilMap.keySet()) {
			...
			Collection<Collection<ProductBundleSale>> cbundlelist = Collections2
					.transform(
							mutillist,
							mobj -> {
								List<ProductBundleSale> newbundlelist = Lists
										.transform(
												bundlelist,
												(ProductBundleSale cbobj) -> {
													/*
													 * ProductBundleSale citem =
													 * Json.fromJson(
													 * Json.toJson(cbobj),
													 * ProductBundleSale.class);
													 */

													ProductBundleSale citem = JSONObject
															.parseObject(
																	JSON.toJSON(
																			cbobj)
																			.toString(),
													...
													// ProductBundleSale citem =
													// new ProductBundleSale();
													// PropertyUtils.copyProperties(citem,
													// cbobj);
