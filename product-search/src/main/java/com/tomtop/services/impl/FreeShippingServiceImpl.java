...
import com.tomtop.services.IFreeShippingService;
 ...
 */
@Service
public class FreeShippingServiceImpl extends BaseService implements IFreeShippingService {

	private static final Logger logger = LoggerFactory
			.getLogger(FreeShippingServiceImpl.class);
	 ...
	 */
	@Override
	public ProductBaseSearchKeyword getProductFreeShipping(String keyword,
		...
		PageBean returnBean = searchIndex.queryByBeanV3(bean,categoryId,"","freeShipping");
		if(returnBean == null){
			return null;
		}
	...
	private void setPageBean(PageBean bean, Integer website,
		...
		filter = new Filter("depots.freeShip",depotName,"&&",false,true);
		bean.getFilters().add(filter);
		
		
		//只过滤 不聚合
	/*	filter = new Filter("depots.freeShipping",true,"&&",false,true);
		bean.getFilters().add(filter);
		
		//处理depotName
		filter = new Filter("depots.depotName",depotName,"&&",false);//只作为过滤
		bean.getFilters().add(filter);*/
	...
	public void setPageBeanTwo(PageBean bean,Integer website,Integer lang,Integer page,Integer size,Integer categoryId,boolean bmain,String depotName){
		...
		filter = new Filter("depots.freeShip",depotName,"&&",true,false);
		bean.getFilters().add(filter);
		...
		bean.getFilters().add(filter);
		//只聚合 不过滤
		filter = new Filter("depots.depotName","","&&",true,false);
		bean.getFilters().add(filter);*/
	...
	private void aggsMapSort(Map<String,List<AggregationEntity>> aggsMap){
		if(aggsMap != null && aggsMap.size() > 0){
			...
			//仓库
			ae = aggsMap.get("depots.freeShip");
			if(ae != null && ae.size() > 0){
				...
				aggsMap.remove("depots.freeShip");
