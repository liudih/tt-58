...
public interface InteractionCommentMapper {
	...
	@Delete("delete from t_interaction_comment where iid=#{0} and cmemberemail=#{1}")
	int deleteInteractionComment(Integer iid,String email);
	
	@Select("SELECT iid,clistingid,csku, cmemberemail,istate,ccomment, foverallrating, iprice, iquality,"
			+ "ishipping,iusefulness, dcreatedate,cplatform,iwebsiteid, ctitle FROM t_interaction_comment where iid=#{0} and cmemberemail=#{1}")
	InteractionComment selectByPrimaryKey(Integer iid,String email);
...
//	List<Foverallrating> countRatings(String listingId);
	...
	@Select("<script>"
			+ "select iid, clistingid,csku , ccomment, iprice, iquality, ishipping, iusefulness, foverallrating, dcreatedate,istate, ctitle  "
	...
	@Select("<script>"
			+ "select count(iid)  "
			+ "from t_interaction_comment where cmemberemail = #{0} and iwebsiteid=#{4} "
			+ "<if test=\"status ==0 \"> and istate=0 </if>"
			+ "<if test=\"status ==1 \"> and istate=1 </if>"
			+ "<if test=\"status ==2 \"> and istate=2 </if> "
			+ "<if test=\"start !=null and end !=null \">  and dcreatedate between #{start} and #{end} </if> "
