...
function fnDialogs(custom){
	...
	if(custom){
		...
		if($('#dialogs').length==0 || $('#dialogs').css('display')=='none'){
			fnfnDialogsShow();
		} else{
			fnfnDialogsHide();
		};
...
function fnfnDialogsHide(){
	//隐藏对话框黑底
	//$('#dialogs').remove();
	$('#dialogs').css('display','none');
	//$('html').css('overflow-y','auto');
};
function fnfnDialogsShow(){
	//显示对话框黑底
	if($('#dialogs').length==0){
		var newElems=$('<div id="dialogs"><!-- 此元素为公用对话框黑底,层级为:1000 --></div>');
