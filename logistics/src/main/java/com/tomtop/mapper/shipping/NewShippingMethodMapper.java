...
public interface NewShippingMethodMapper {
	...
	List<ShippingMethod> getShippingMethod(
			@Param("storageId") Integer storageId,
			@Param("country") String country, @Param("weight") Double weight,
			/*@Param("lang") Integer lang, @Param("price") Double price,*/
			@Param("isSpecial") boolean isSpecial,
	...
	@Select("select * from shipping_method where iid = #{id}")
	ShippingMethod getShippingMethodById(Integer id);

	@Update("update shipping_method set benabled = #{1} where ccode = #{0}")
	Integer updateEnableByCode(String ccode, boolean benabled);
	
	
	@Insert("insert into shipping_method (istorageid, benabled,bexistfree,ffreebeginprice,ffreeendprice,"
